    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/60">
        ...
        <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
          ...
          в настройках аккаунта. Токен предоставляется в формате <code className="rounded bg-slate-100 px-1 py-0.5 dark:bg-slate-800">tokenId.secret</code>{' '}
        ...
        <div className="mt-4 space-y-2 rounded-xl bg-slate-100 p-4 text-xs font-mono text-slate-700 dark:bg-slate-800 dark:text-slate-200">
          ...
          <pre className="max-w-full overflow-x-auto whitespace-pre-wrap break-words rounded-lg bg-slate-900/90 p-3 text-[0.8rem] leading-relaxed text-slate-100 shadow-inner dark:bg-slate-950/70">
{String.raw`curl -H "X-Account-Token: <tokenId.secret>" \
  ...
  "http://localhost:8000/api/repos/<repoId>/file?path=README.md&branch=main"`}
      ...
      <section className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/60">
        ...
        <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
          ...
          Ограничения приватности репозиториев соблюдаются автоматически, один токен покрывает все проекты, к которым у вас есть доступ.
        </p>
        <ul className="mt-4 list-disc space-y-2 pl-6 text-sm text-slate-600 dark:text-slate-300">
          ...
          <li>Все эндпоинты поддерживают заголовок <code>X-Account-Token</code> или <code>Authorization: Token ...</code>.</li>
        </ul>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/60">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-6 text-sm text-slate-600 dark:text-slate-300">
          <li>Зарегистрируйтесь и создайте токен в разделе <em>Аккаунт → Персональные токены</em>.</li>
        ...
        <div className="mt-4 max-w-full overflow-x-auto whitespace-pre-wrap break-words rounded-xl bg-slate-100 p-4 text-xs font-mono leading-relaxed text-slate-700 dark:bg-slate-800 dark:text-slate-200">
{String.raw`curl -X PUT "http://localhost:8000/api/repos/<repoId>/file" \
  -H "X-Account-Token: <tokenId.secret>" \
